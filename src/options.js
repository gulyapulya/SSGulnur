...
function ssgulnur(options) {
  ...
  if (options.help) {
    ...
    console.log(
      chalk.green.dim('ssgulnur -i | --input <source>') +
        chalk.gray(' specify a file or folder to use')
    );
    console.log(chalk.green('Options for input:'));
    console.log(
      chalk.green.dim('-o | --output <folder>') +
        chalk.gray(' specify an output folder for produced html')
    );
    console.log(
      chalk.gray('By default, the output folder would be ./dist unless specified otherwise')
    );
    console.log(
      chalk.green.dim('-s | --stylesheet <url>') + chalk.gray(' specify a stylesheet url to use')
    );
    console.log(chalk.gray('For example, https://cdn.jsdelivr.net/npm/water.css@2/out/water.css'));
    console.log(chalk.green('Config:'));
    console.log(
      chalk.green.dim('-c | --config <file>') + chalk.gray(' specify a config file to use')
    );
    return;
  }
